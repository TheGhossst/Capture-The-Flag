...
export default function InvisibleInkChallenge() {
    ...
    useEffect(() => {
        async function checkStatus() {
            try {
                ...
                const invisibleInkCategory = data.find(cat =>
                    cat.challenges.some(c => c.id === 5)
                )

                if (invisibleInkCategory) {
                    const challenge = invisibleInkCategory.challenges.find(c => c.id === 5)
                    if (challenge) {
                        setQuestionData({
                            title: challenge.title,
                            description: challenge.description,
                            category: invisibleInkCategory.category,
    ...
    const unlockHint = async () => {
        ...
        try {
            const response = await fetch('/api/questions/5/hint', {
                method: 'POST',
                credentials: 'include',
            })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('/api/questions/5/submit', {
                method: 'POST',
            ...
            if (data.success) {
                ...
                setError(`You found the hidden flag!`)
                setTimeout(() => {
                    router.refresh()
                    ...
                    <div className="space-y-6">
                        <div className="bg-[#171B26] p-6 rounded-lg border border-gray-800">
                            ...
                            <p className="text-gray-400">
                                ...
                                <span className="text-[#171B26]">{"flag{invisible_flag_found}"}</span>
                            </p>
                        </div>
